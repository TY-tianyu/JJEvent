   ...
   // SQLite 采用串行模型，所有线程都公用同一个数据库连接。
        ...
        db = FinalDb.create(EventManager.getContext(), Constant.DB_NAME, false, Constant.DB_VERSION,
                new FinalDb.DbUpdateListener() {
                    ...
                    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
                        ...
                        Logger.logWrite(Constant.TAG, "onUpgrade ,delete DB_NAME success!-->");
    ...
    public static synchronized List<EventBean> getEventListByDate(String formateDate) {
        ...
        try {
            ...
            Logger.logWrite(Constant.TAG, "getEventListByDate  success!-->"+formateDate+"--resultList.size()--"+resultList.size());

        } catch (Exception e) {
            e.printStackTrace();
            Logger.logWrite(Constant.TAG, "getEventListByDate  failed-->"+e.getMessage());
    ...
    public static synchronized List<EventBean> getEventListByLimit(int  start ,int end){
        ...
        try {
            ...
            Logger.logWrite(Constant.TAG, "getEventListByLimit  success!-->第"+start+"条到"+end+"之间--resultList.size()--"+resultList.size());

        } catch (Exception e) {
            e.printStackTrace();
            Logger.logWrite(Constant.TAG, "getEventListByLimit  failed-->"+e.getMessage());
    ...
    public static synchronized void deleteEventListByLimit(int  start ,int end){
        ...
        try {
            ...
            Logger.logWrite(Constant.TAG, "deleteEventListByLimit  success!-->第"+start+"条到"+end+"之间--resultList.size()--"+resultList.size());

        } catch (Exception e) {
            e.printStackTrace();
            Logger.logWrite(Constant.TAG, "deleteEventListByLimit  failed-->"+e.getMessage());
    ...
    public static synchronized int  getEventRowCount() {
        ...
        try {
            ...
            Logger.logWrite(Constant.TAG, "getEventRowCount  success!-->"+resultCount);

        } catch (Exception e) {
            e.printStackTrace();
            Logger.logWrite(Constant.TAG, "getEventRowCount  failed-->"+e.getMessage());
        }
    ...
    public static synchronized void deleteEventListByDate(String formateDate) {
        ...
        try {
            ...
            Logger.logWrite(Constant.TAG, "deleteEventListByDate  success!-->"+formateDate);

        } catch (Exception e) {
            e.printStackTrace();
            Logger.logWrite(Constant.TAG, "deleteEventListByDate  failed-->"+e.getMessage());
        }
    ...
    public static synchronized boolean addEventData(EventBean data) {
        try {
            ...
            Logger.logWrite(Constant.TAG, "save to db success-->"+data.toString());

            return true;
        } catch (Exception e) {
            e.printStackTrace();
            Logger.logError(Constant.TAG, "save to db failed-->"+e.getMessage());
            if (e.getMessage().contains("no column named")){
                db.dropTable(EventBean.class);
                //table eventlist has no column named ds
                Logger.logWrite(Constant.TAG, "has no column named,so dropTable");
                db.save(data);
                Logger.logWrite(Constant.TAG, "reload : save to db success-->");
    ...
    public static boolean clearAllCache() {
        try {
           // db.deleteAll(EventBean.class);
            ...
            Logger.logWrite(Constant.TAG, "-clearAllCache:" + e.toString());
        }
        return false;
    }
    ...
    public static synchronized boolean addData(EventBean data) {
        try {
            ...
            Logger.logWrite(Constant.TAG, "-addListCache" + e.toString());
            return false;
        }
    }
    ...
    public static boolean deleteData(String id) {
        try {
            ...
            Logger.logWrite(Constant.TAG, "-deletelistCache" + e.toString());
            return false;
        }
    }
    ...
    public static synchronized boolean updateData(EventBean data) {
        try {
            ...
            Logger.logWrite(Constant.TAG, "-updateRowInfoById" + e.toString());
            return false;
        }
    }
    ...
    public static synchronized EventBean findData(String id) {
        try {
            ...
            Logger.logWrite(Constant.TAG, "-findRowInfoById" + e.toString());
            return null;
        }
    }
...
//            db.deleteByWhere(EventBean.class, " exposed_id=\"" + ExposedID + "\"");
            Logger.logWrite(Constant.TAG, "getEventByExposedID  success!-->"+ExposedID+"--resultList.size()--"+resultList.size());

        } catch (Exception e) {
            e.printStackTrace();
            Logger.logWrite(Constant.TAG, "getEventByExposedID  failed-->"+e.getMessage());
            if (e.getMessage().contains("no column named")){
                db.dropTable(EventBean.class);
                //table eventlist has no column named ds
                Logger.logWrite(Constant.TAG, "has no column named,so dropTable");
    ...
    public static synchronized boolean updateEventBean(EventBean bean, String ExposedID) {
        try {
            ...
            Logger.logWrite(Constant.TAG, "-updateEventBean  sucess ,exposed_id=" + ExposedID);

            return true;
        } catch (Exception e) {
            Logger.logWrite(Constant.TAG, "-updateRowInfoBy exposed_id" + e.toString());
            return false;
        }
    }
